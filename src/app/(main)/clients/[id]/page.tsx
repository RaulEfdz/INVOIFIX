...
import type { Client, Invoice, ClientDocument, ClientProject } from "@/types"; // Updated Client type
import { DUMMY_CLIENTS, DUMMY_INVOICES } from "@/lib/constants"; // Updated DUMMY_CLIENTS
import { ArrowLeft, Edit, FileText, Briefcase, DollarSign, Activity, FileArchive, UploadCloud, LinkIcon, MapPin, Building, Phone, Globe, Info, CalendarDays, Users } from "lucide-react";
...
export default function ClientDetailPage() {
  ...
  const clientId = params.id as string; // Renamed to clientId
  
  const [client, setClient] = React.useState<Client | null>(null); // Updated type
  const [invoices, setInvoices] = React.useState<Invoice[]>([]);
  const [documents, setDocuments] = React.useState<ClientDocument[]>([]); // Updated type
  const [projects, setProjects] = React.useState<ClientProject[]>([]); // Updated type

  React.useEffect(() => {
    const foundClient = DUMMY_CLIENTS.find(c => c.id === clientId); // Updated constant and variable
    if (foundClient) {
      setClient(foundClient);
      const clientInvoices = DUMMY_INVOICES.filter(inv => inv.clientId === clientId); // Updated field
      setInvoices(clientInvoices);
      setDocuments(DUMMY_DOCUMENTS.filter(doc => doc.clientId === clientId)); // Updated field
      setProjects(DUMMY_PROJECTS.filter(proj => proj.clientId === clientId)); // Updated field
    } else {
      // router.push('/clients'); 
    }
  }, [clientId, router]);
  ...
  if (!client) {
    return (
      <>
        <AppHeader pageTitle="Client Details" />
  ...
  const calculateClientAge = (createdAt: string) => { // Renamed function
