  ...
  return (
    <Card className="shadow-sm hover:shadow-md transition-shadow bg-card rounded-xl">
      <CardContent className="p-4 space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <Avatar className="h-10 w-10 border">
              <AvatarImage 
                src={customer.avatarUrl || `https://placehold.co/100x100.png?text=${getInitials(customer.firstName, customer.lastName)}`} 
            ...
            <div className="overflow-hidden">
              <p className="font-semibold text-sm text-foreground truncate" title={`${customer.firstName} ${customer.lastName}`}>
                {customer.firstName} {customer.lastName}
              </p>
              <p className="text-xs text-muted-foreground font-light truncate" title={customer.email}>
                {customer.email}
              </p>
            </div>
          </div>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="font-light">
              <DropdownMenuItem onSelect={() => window.location.href = `/customers/${customer.id}`}>
                View Details
              </DropdownMenuItem>
        ...
        <div className="pt-1">
          <Link href={`/customers/${customer.id}`} className="flex items-center justify-end text-xs text-primary hover:text-primary/80 font-medium transition-colors">
