...
export function TicketDetailSheet({
  ...
  const [currentTicket, setCurrentTicket] = React.useState<Ticket | null>(
    ticket
  );
  const [newStatus, setNewStatus] = React.useState<TicketStatus | undefined>(
    ticket?.status
  );
  const [comment, setComment] = React.useState("");
  const [comments, setComments] = React.useState<
    Array<{ text: string; date: Date; author: string }>
  >([]);
  ...
  const handleRemoveChecklistItem = (itemId: string) => {
    const updatedChecklist = (currentTicket.checklist || []).filter(
      (item) => item.id !== itemId
    );
    const updatedTicketData = { ...currentTicket, checklist: updatedChecklist };
    setCurrentTicket(updatedTicketData);
    ...
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-3xl w-full p-0 flex flex-col">
        <SheetHeader className="p-6 border-b">
          ...
          <SheetDescription className="font-light text-muted-foreground">
            {currentTicket.title}
          </SheetDescription>
        </SheetHeader>
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                <Card className="shadow-lg rounded-xl">
                  ...
                  <CardContent className="p-6 space-y-4">
                    <div>
                      <h4 className="font-semibold text-md text-foreground mb-1">
                        Descripción
                      </h4>
                      <p className="font-light text-sm text-muted-foreground whitespace-pre-wrap">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                      <div className="flex items-center">
                        ...
                        <span className="font-light text-muted-foreground">
                          {currentTicket.submittedBy} ({currentTicket.role})
                        </span>
                      </div>
                      <div className="flex items-center">
                        <UserCircle className="h-4 w-4 mr-2 text-muted-foreground" />
                        ...
                        <span className="font-light text-muted-foreground">
                          {currentTicket.assignedTo || "Sin asignar"}
                        </span>
                      </div>
                      <div className="flex items-center">
                      ...
                      {currentTicket.clientId && (
                        <div className="flex items-center">
                          ...
                          <a
                            href={`/clients/${currentTicket.clientId}`}
                ...
                <Card className="shadow-lg rounded-xl">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {currentTicket.checklist &&
                    currentTicket.checklist.length > 0 ? (
                      <ul className="space-y-2 max-h-48 overflow-y-auto pr-1">
                        {currentTicket.checklist.map((item) => (
                          <li
                            key={item.id}
                            className="flex items-center justify-between p-2 bg-secondary/30 rounded-md hover:bg-secondary/50 group"
                          >
                ...
                <Card className="shadow-lg rounded-xl">
                  <CardHeader>
                    ...
                    <CardDescription className="font-light">
                      Tasks associated with this ticket.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {currentTicket.relatedTasks &&
                    currentTicket.relatedTasks.length > 0 ? (
                      <ul className="space-y-2">
                        {currentTicket.relatedTasks.map((task) => (
                          <li
                            key={task.id}
                            className="text-sm font-light text-muted-foreground p-2 bg-secondary/30 rounded-md hover:bg-secondary/50"
                          >
                            <LinkIcon className="h-3 w-3 inline mr-1.5" />{" "}
              ...
              <div className="lg:col-span-1 space-y-6">
                  ...
                  <CardContent className="p-6 space-y-4">
                    <div className="text-center mb-2">
                      <Badge
                        className={cn(
                          ...
                          getStatusColorClass(currentTicket.status),
                          getStatusColorClass(currentTicket.status).includes(
                            "dark:bg-sky-800"
                          ) ||
                            getStatusColorClass(currentTicket.status).includes(
                              "dark:bg-amber-800"
                            ...
                            : getStatusColorClass(
                              ...
                              getStatusColorClass(
                                currentTicket.status
                              ).includes("bg-amber-100") ||
                              getStatusColorClass(
                                currentTicket.status
                              ).includes("bg-indigo-100")
                            ? "text-sky-800 dark:text-sky-100"
                            : getStatusColorClass(
                                currentTicket.status
                              ).includes("bg-emerald-100")
                            ? "text-emerald-800 dark:text-emerald-100"
                            : getStatusColorClass(
                                currentTicket.status
                              ).includes("bg-slate-200")
                    ...
                    <div>
                      ...
                      <Select
                        value={newStatus}
                        onValueChange={(value) =>
                          setNewStatus(value as TicketStatus)
                        }
                      >
                        <SelectTrigger
                          id="ticketStatus"
                          className="mt-1 font-light bg-card"
                        >
                    ...
                    <Button
                      ...
                      disabled={newStatus === currentTicket.status}
                    >
                      Update Status
                    </Button>
                  </CardContent>
                ...
                <Card className="shadow-lg rounded-xl sticky top-[calc(theme(spacing.6)_+_260px)]">
                  {" "}
                  {/* Adjust top based on previous card height */}
                  <CardHeader>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    >
                      <LinkIcon className="mr-2 h-4 w-4" /> Attach File
                    </Button>
                    <Button
                      variant="outline"
                      className="w-full justify-start font-light"
            ...
            <Card className="shadow-lg rounded-xl">
              <CardHeader>
                ...
                <CardDescription className="font-light">
                  Add notes, updates, or the solution for this ticket.
                </CardDescription>
              </CardHeader>
                ...
                {comments.length > 0 && (
                  <div className="space-y-3 max-h-60 overflow-y-auto pr-2 border-b pb-3 mb-3">
                    {comments.map((c, index) => (
                      <div
                        key={index}
                        className="p-3 bg-secondary/50 rounded-md border text-sm"
                      >
                        <p className="font-light text-muted-foreground whitespace-pre-wrap">
                          {c.text}
                        </p>
